...
def decide_winner(move1, move2):
    if move1 == move2:
        return 0
    # Các trường hợp Người chơi 1 THẮNG:
    # 1. Búa thắng Kéo
    # 2. Kéo thắng Bao
    # 3. Bao thắng Búa
    wins = [("bua", "keo"), ("keo", "bao"), ("bao", "bua")]
    
    if (move1, move2) in wins:
            ...
            while m1 is None or m2 is None:
                try:
                    while not q1.empty():
                        ...
                        elif data is None: raise Exception("DISCONNECT_P1")
                        elif data.startswith("MOVE "): m1 = data
                except Empty: pass
                ...
                try:
                    while not q2.empty():
                        ...
                        elif data is None: raise Exception("DISCONNECT_P2")
                        elif data.startswith("MOVE "): m2 = data
                except Empty: pass
...
def client_thread(conn, addr):
    ...
    try:
        ...
        with clients_lock:
            clients[conn] = {"name": name, "status": "IDLE", "queue": Queue()}
        ...
        while True:
            ...
            status = "IDLE"
            with clients_lock:
                if conn in clients: status = clients[conn]["status"]
            
            if status == "BUSY":
                with clients_lock:
                    if conn in clients: clients[conn]["queue"].put(line)
            else:
                if line.startswith("INVITE "):
                    ...
                    else: send_line(conn, f"ERROR {t_name} busy or offline")
                elif line.startswith("INVITE_RESP "):
                    parts = line.split()
                    if len(parts) >= 3:
                        sender, ans = parts[1], parts[2]
                        s_conn, s_info = find_client_by_name(sender)
                        if s_conn:
                            if ans == "NO": send_line(s_conn, f"INVITE_REJECT {name}")
                            ...
                            else: send_line(conn, "ERROR Too late")
    finally:
        with clients_lock:
...
def main():
    ...
    s.bind((HOST, PORT))
    s.listen(); print(f"Listening on {PORT}")
    while True:
        try:
            conn, addr = s.accept()
